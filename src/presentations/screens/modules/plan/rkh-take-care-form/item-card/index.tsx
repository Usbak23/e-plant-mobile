...
interface IRKHTakeCareItemCardProps {
  item?: {
    roleId: string
    qty: number
    cost: number
...
const RKHTakeCareItemCard: React.FC<IRKHTakeCareItemCardProps> = props => {
  ...
  return (
    <View style={styles.card}>
      <View style={{flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center'}}>
        <View style={{flex: 1}}>
          <Text>{props.item?.name || props.item?.item?.name || '-'}</Text>
        </View>
...
export default RKHTakeCareItemCard

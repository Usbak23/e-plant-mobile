...
const PMAReport = () => {
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `r-pma_${millis}.${fileType}`
  ...
  const pmaReport = useSelector((state: RootStateType) => state.dashboardAndChart?.pmaReport)
  const iframeUrl = pmaReport?.data?.data?.response?.iframe
  ...
  const getData = useCallback(data => {
    dispatch(actions.getPMAReport.request({ loading: true, data: { ...data } }))
  }, [])

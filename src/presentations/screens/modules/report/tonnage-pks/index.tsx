...
const TonnagePKSReport = () => {
  ...
  const tonnagePKSReport = useSelector((state: RootStateType) => state.dashboardAndChart?.tonnagePKSReport)
  const iframeUrl = tonnagePKSReport?.data?.data?.response?.iframe
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `tonase_pks_${millis}.${fileType}`
  ...
  const ChartView = () => (
    <View>
      ...
      {Boolean(tonnagePKSReport?.loading) && (
  ...
  const getData = useCallback(data => {
    dispatch(actions.getTonnagePKSReport.request({ loading: true, data: { ...data } }))
  }, [])
  ...
  return (
    <SafeAreaView style={styles.root}>
      <Header title="Laporan Tonase PKS" />
      ...
      <ModalFilter
        ...
        <Button style={{ width: '100%', marginTop: 10, backgroundColor: theme.colors.redDark }} onPress={() => {
          setQuery({ organizationId: '', year: '', month: '', date: '', search: '', divisionId: '' })
        }}>
...
export default TonnagePKSReport

...
const RKBTakeCareReport = () => {
  ...
  const rkbTakeCareReport = useSelector((state: RootStateType) => state.dashboardAndChart?.rkbTakeCare)
  const iframeUrl = rkbTakeCareReport?.data?.data?.response?.iframe
  ...
  const getData = useCallback(data => {
    dispatch(actions.getRKBTakeCareReport.request({loading: true, data: {...data}}))
  }, [])
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `rkb_rawat_${millis}.${fileType}`
  ...
  const ChartView = () => (
    <View style={{paddingBottom: 16}}>
      ...
      {Boolean(rkbTakeCareReport?.loading) && isValidForm() && (
  ...
  return (
    <SafeAreaView style={styles.root}>
      <Header title="Laporan RKB Rawat" />
...
export default RKBTakeCareReport

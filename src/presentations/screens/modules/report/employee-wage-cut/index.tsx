...
const EmployeeWageCutReport = () => {
  ...
  const employeeWageCutReport = useSelector(
    (state: RootStateType) => state.dashboardAndChart?.employeeWagesDeductionReport,
  )
  const iframeUrl = employeeWageCutReport?.data?.response?.iframe
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `potongan_karyawan_${millis}.${fileType}`
  ...
  const getData = useCallback(data => {
    dispatch(actions.getEmployeeWagesDeductionReport.request({loading: true, data: {...data}}))
  }, [])
  ...
  const ChartView = () => (
    <View>
      ...
      {Boolean(employeeWageCutReport?.loading) && isValidForm() && (
  ...
  return (
    <SafeAreaView style={styles.root}>
      <Header title="Potongan Karyawan" />
...
export default EmployeeWageCutReport

...
const CropBookReport = () => {
  ...
  const cropBookReport = useSelector((state: RootStateType) => state.dashboardAndChart?.cropBookReport)
  const iframeUrl = cropBookReport?.data?.data?.response?.iframe
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `cropbook_${millis}.${fileType}`
  ...
  const getData = useCallback(data => {
    dispatch(actions.getCropBookReport.request({ loading: true, data: { ...data } }))
  }, [])
  ...
  const ChartView = () => (
    <View>
      ...
      {Boolean(cropBookReport?.loading) && isValidForm() && (

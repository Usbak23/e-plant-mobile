...
const YieldReport = () => {
  ...
  const yieldTableChart = useSelector((state: RootStateType) => state.dashboardAndChart?.yieldReport)
  const iframeUrl = yieldTableChart?.data?.response?.iframe

  const divisions = organizations.find(v => v.value == query.organizationId)?.divisions || []

  const isValidForm = () => {
    if (query?.date) {
      return Boolean(query?.organizationId && query?.divisionId)
    } else {
  ...
  const constructTitleHelper = () => {
    ...
    const monthsInStr = months.find((m: any) => m.value == query?.month)?.label || ''
    return o + ' - ' + d + ' - ' + (query?.date ? query?.date : `${monthsInStr} ${query.year}`)
  }
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `yield_${millis}.${fileType}`
  ...
  const ChartView = () => (
    <View>
      ...
      {Boolean(yieldTableChart?.loading) && (
  ...
  const getData = useCallback(data => {
    dispatch(actions.getYieldReport.request({ loading: true, data: { ...data } }))
  }, [])

...
const RRPReport = () => {
  ...
  const [query, setQuery] = useState({
    ...
    date: q?.date || q?.restanParamsDate || '',
  })

  const [isDatePickerOpen, setIsDatePickerOpen] = useState<boolean>(false)
  const rrpReport = useSelector((state: RootStateType) => state.dashboardAndChart?.rrpReport)
  const iframeUrl = rrpReport?.data?.data?.response?.iframe
  ...
  const getData = useCallback(data => {
    dispatch(actions.getRRPReport.request({ loading: true, data: { ...data } }))
  }, [])
  ...
  const handleExport = async (url: string, fileType: string) => {
    try {
      ...
      const fileName = `rrp_${millis}.${fileType}`
  ...
  const ChartView = () => (
    <View>
      ...
      {Boolean(rrpReport?.loading) && isValidForm() && (
